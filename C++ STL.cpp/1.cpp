#include<bits/stdc++.h> 
using namespace is just a scope 
agr kch use krna h to phle declare kro then kro
namespace raj{
int val=50;
}
int main(){
...
//  this is how we create self datz type
struct node {
	string str; 
	int num;
	double doub; 
...
// if you will define this globally then its vlaue will be zero nahi to garbagge value hogi agr main ke andr declare krogwe toh
...
//  we can take arr double char and can be declared in self data
	// int 
	// double 
	// char
...
//  agr phle tmne kch bi declare krlo ki ye 1 h toh bus phla iondex 1 hoga baaki 0 hogaa
...
//  acccesing these arrays by using ".at"  this is container array
	arr.at(index); 
    ...
    // begin() it is pointing to the start
	// rbegin() is  beginning from last
   //  end() point to right after the last element
   //  rend()   right before the first point
	array<int, 5> arr = {1, 3, 4, 5, 6}; 
	for(auto it=arr.begin(); it!=arr.end();it++) {
        // as it is the pointer(arr.bgein) so we use * to acces the value
		cout << *it << " "; 
	}
	...
	arr.pop_back(); // {0} removes the element from behind 
	...
	vector<int> raj1;
    // raj.emplace_back(1) it is faster than raj.push_back(1) 
	raj1.push_back(1); 
	raj1.push_back(2); 
...
//  this is tradiitonal way of writing
